    ...
    public class MainActivity : Activity
    {
        protected override void OnCreate(Bundle bundle)
        {
            ...
            button.Click += delegate
            {
                ...
                // Begin the asynchronous request to the server. The callback delegate (MyCallback,
                // below) handles the HTTP response.
                IAsyncResult result = (IAsyncResult)request.BeginGetResponse(MyCallback, request);
            };
        }

        // The async callback delegate: parse the JSON response and update the UI with results.
