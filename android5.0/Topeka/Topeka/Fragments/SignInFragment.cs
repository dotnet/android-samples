	...
	public class SignInFragment : Fragment
	{
		...
		void CheckIsInEditMode() {
			...
			if (null == arguments) {
				...
				edit = arguments.GetBoolean(ArgEdit, false);
			}
		}
        ...
        void InitContentViews(View view)
        {
            ...
            firstName.TextChanged += (sender, e) =>
            {
                if (string.Concat(e.Text).Length == 0)
                {
                    doneFab.Visibility = ViewStates.Gone;
                }
                else
                {
                    doneFab.Visibility = ViewStates.Visible;
                }
            };
            lastInitial = view.FindViewById<EditText>(Resource.Id.last_initial);
            lastInitial.TextChanged += (sender, e) =>
            {
                if (string.Concat(e.Text).Length == 0)
                {
                    doneFab.Visibility = ViewStates.Gone;
