...
vec4[4] points = vec4[4] (
	vec4 (-0.5, -0.5 / sqrt (3.0), -0.5 / sqrt (6.0), 1.0),
	vec4 (0.5, -0.5 / sqrt (3.0), -0.5 / sqrt (6.0), 1.0),
	vec4 (0, 1.0 / sqrt (3.0), -0.5 / sqrt (6.0), 1.0),
	vec4 (0, 0, 1.5 / sqrt (6.0), 1.0)
);
...
void tetrahedron (uint idx, uint tidx, vec4 off)
{
	...
	vertices.data [idx].normal = vec4 (0, 0, -1.0, 1.0);
	vertices.data [idx + 1u].normal = vec4 (0, 0, -1.0, 1.0);
	vertices.data [idx + 2u].normal = vec4 (0, 0, -1.0, 1.0);
	vertices.data [idx].baycentric = vec3 (1.0, 0, 0);
	vertices.data [idx + 1u].baycentric = vec3 (0, 1.0, 0);
	vertices.data [idx + 2u].baycentric = vec3 (0, 0, 1.0);

	triangles.triangle [tidx + 1u].a = 4u + idx;
	triangles.triangle [tidx + 1u].b = 5u + idx;
	triangles.triangle [tidx + 1u].c = 3u + idx;
	vertices.data [idx + 4u].normal = vec4 (0, -2.0 / sqrt (6.0), 0.5 / sqrt (3.0), 1.0);
	vertices.data [idx + 5u].normal = vertices.data [idx + 4u].normal;
	vertices.data [idx + 3u].normal = vertices.data [idx + 4u].normal;
	vertices.data [idx + 4u].baycentric = vec3 (1.0, 0, 0);
	vertices.data [idx + 5u].baycentric = vec3 (0, 1.0, 0);
	vertices.data [idx + 3u].baycentric = vec3 (0, 0, 1.0);

	triangles.triangle [tidx + 2u].a = 8u + idx;
	triangles.triangle [tidx + 2u].b = 7u + idx;
	triangles.triangle [tidx + 2u].c = 6u + idx;
	vertices.data [idx + 8u].normal = vec4 (-3.0 / (sqrt (3.0) * sqrt (6.0)), 1.0 / sqrt (6.0), 0.5 / sqrt (3.0), 1.0);
	vertices.data [idx + 6u].normal = vertices.data [idx + 8u].normal;
	vertices.data [idx + 7u].normal = vertices.data [idx + 8u].normal;
	vertices.data [idx + 8u].baycentric = vec3 (1.0, 0, 0);
	vertices.data [idx + 6u].baycentric = vec3 (0, 1.0, 0);
	vertices.data [idx + 7u].baycentric = vec3 (0, 0, 1.0);

	triangles.triangle [tidx + 3u].a = 9u + idx;
	triangles.triangle [tidx + 3u].b = 10u + idx;
	triangles.triangle [tidx + 3u].c = 11u + idx;
	vertices.data [idx + 9u].normal = vec4 (3.0 / (sqrt (3.0) * sqrt (6.0)), 1.0 / sqrt (6.0), 0.5 / sqrt (3.0), 1.0);
	vertices.data [idx + 10u].normal = vertices.data [idx + 9u].normal;
	vertices.data [idx + 11u].normal = vertices.data [idx + 9u].normal;
	vertices.data [idx + 9u].baycentric = vec3 (1.0, 0, 0);
	vertices.data [idx + 10u].baycentric = vec3 (0, 1.0, 0);
	vertices.data [idx + 11u].baycentric = vec3 (0, 0, 1.0);
}
